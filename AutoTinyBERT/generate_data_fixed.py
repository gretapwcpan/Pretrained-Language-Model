# coding=utf-8
# Fixed version of generate_data.py
...
class DocumentDatabase:
    def __init__(self, reduce_memory=False, cache_dir=None):
        if reduce_memory:
            self.temp_dir = TemporaryDirectory()
            self.working_dir = Path(self.temp_dir.name)
            self.document_shelf_filepath = self.working_dir / 'shelf.db'
            # FIX: Use the temp directory path instead of hardcoded /cache
            shelf_path = str(self.document_shelf_filepath)
            self.document_shelf = shelve.open(shelf_path,
                                              flag='n', protocol=-1)
            self.documents = None
        else:
            self.documents = []
...
def main():
    ...
    parser.add_argument("--num_files", type=int, default=4,
                        help="Number of output files to generate (default: 4, was 28)")
    parser.add_argument("--min_examples_per_file", type=int, default=100,
                        help="Minimum examples per file to avoid empty files")
    ...
    # Pass cache_dir to DocumentDatabase
    cache_dir = os.environ.get('CACHE_DIR', '/tmp')
    with DocumentDatabase(reduce_memory=args.reduce_memory, cache_dir=cache_dir) as docs:
        ...
        # Dynamically adjust number of files based on data size
        file_num = min(args.num_files, max(1, len(all_examples) // args.min_examples_per_file))
        logger.info(f'Writing to {file_num} files')
        ...
        # Distribute examples evenly across files
        for idx, instance in enumerate(all_examples):
            file_idx = idx % file_num
            fouts[file_idx].write(json.dumps(instance) + '\n')

        # Close all files and report statistics
        for i, fout in enumerate(fouts):
            ...
            logger.info(f'File {i}: {line_count} examples')
